...
const Home: NextPage = () => {
  ...
  const isIntersecting = useIntersectionObserver(ref);
	...
	const { data, error, isValidating, setSize, mutate } = useSWRInfinite<any>(getKey, { revalidateFirstPage: false, revalidateOnFocus: false });
	const [ createTweet, { loading, data: mutateData, error: mutateError } ] = useMutation<MutationResult>('/api/tweets');
	const [ logout, { data: logoutData, error: logoutError } ] = useMutation<{ok: boolean}>('/api/users/log-out');
  ...
  const isEmpty = data?.[0]?.tweets?.length === 0;
	...
	const profilePopup = [ { title: '프로필 보기', onClickFn: () => router.push('/profile') }, { title: '로그아웃', onClickFn: () => logout({}) } ];
	return (
		<div className="min-h-screen bg-black">
				<div className="flex">
					...
					<section className="z-10 hidden sm:block top-0 left-0 fixed h-full w-[80px] lg:w-[300px] text-white  py-4">
						...
						<svg viewBox="0 0 24 24" className="mx-auto h-10 w-10 text-white" fill="currentColor">
							<g>
								<path
						...
						<nav className="mt-5 px-2 space-y-1">
							<a href="#"
								...
								<svg className="lg:mr-4 h-6 w-6 " stroke="currentColor" fill="none" viewBox="0 0 24 24">
							...
							<a href="#"
								className="group flex justify-center lg:justify-start items-center px-2 py-2 text-base leading-6 font-semibold rounded-full hover:bg-blue-800 hover:text-blue-300">
								<svg className="lg:mr-4 h-6 w-6" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
							...
							<div className='flex justify-center'>
								<button
									className="bg-blue-400 lg:w-48 mt-5 hover:bg-blue-600 text-white font-bold py-2 px-2 lg:px-4 rounded-full">
									<span className='hidden lg:block'>Tweet</span>
									<svg viewBox="0 0 24 24" aria-hidden="true" fill='#fff' className="w-4 h-4 block lg:hidden">
										<g>
											<path
						...
						<div onClick={togglePopup} className="absolute bottom-0 right-0 left-0  mx-2  mb-5 ">
							<div className="p-2 rounded-full hover:bg-blue-400 group block">
								<div className="flex justify-center lg:justify-start items-center">
									...
									<div className="hidden lg:block lg:ml-3">
										...
										<p
											className="text-xs leading-5 font-medium text-gray-400 group-hover:text-gray-300 transition ease-in-out duration-150">
											@nickname
										</p>
									</div>
									<svg className="ml-3 hidden lg:block w-6" viewBox="0 0 24 24" aria-hidden="true" fill="#fff">
										<g>
							...
							<div className={cls(" absolute -top-[120px] lg:-top-[135px] left-[175px] lg:left-1/2 -translate-x-1/2 ", popupOn ? 'block' : 'hidden')}> 
					...
					<section className="sm:ml-[80px] lg:ml-[300px] w-full lg:w-3/5 border-x border-gray-700 h-auto">
						...
						return <TweetBox key={tweet.id} id={tweet.id} userId={tweet.user.id} userName={tweet.user.name} payload={tweet.payload}
							updatedAt={tweet.updatedAt} likes={tweet._count.like} />
						})}
    ...
    <span className="sr-only">Loading...</span>
					...
					<section className="w-2/5 hidden lg:block">
						...
						<div className="relative text-gray-300 w-80 p-3 pb-0 mr-16">
							<div className="absolute ml-4 mt-3 lg:mr-4">
							...
							<input type="text" name="search" placeholder="Search Twitter"
								className="bg-gray-800 h-10 px-10 pr-5 w-full rounded-full text-sm focus:outline-none bg-purple-white shadow border-0" />
						</div>
						...
						<div className="max-w-sm rounded-lg bg-gray-800 overflow-hidden shadow-lg m-4 mr-20">
							<div className="flex">
								...
								<div className="flex-1 px-4 py-2 m-2">
									<a href=""
										...
										<svg className="m-2 h-6 w-6" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
											stroke="currentColor" viewBox="0 0 24 24">
											<path
							...
							<div className="flex">
								<div className="flex-1">
									<p className="px-4 ml-2 mt-3 w-48 text-xs text-gray-400">Trending</p>
								...
								<div className="flex-1 px-4 py-2 m-2">
									<a href=""
										...
										<svg className="m-2 h-5 w-5" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
											stroke="currentColor" viewBox="0 0 24 24">
							...
							<div className="flex">
								<div className="flex-1">
									<p className="px-4 ml-2 mt-3 w-48 text-xs text-gray-400">Trending</p>
									<h2 className="px-4 ml-2 w-48 font-bold text-white">#Typescript</h2>
							...
							<div className="flex">
								<div className="flex-1">
									<p className="px-4 ml-2 mt-3 w-48 text-xs text-gray-400">Trending</p>
									<h2 className="px-4 ml-2 w-48 font-bold text-white">#Next.js</h2>
							...
							<div className="flex">
								<div className="flex-1 p-4">
									<h2 className="ml-2 w-48 font-bold text-sm text-blue-400">Show more</h2>
								</div>
							</div>
									...
									<div className="flex items-center w-48">
										<div>
											<img className="inline-block h-10 w-auto rounded-full ml-4 mt-2"
												src="http://placeimg.com/640/480/any" alt="" />
										</div>
						...
						<div className="flow-root m-6">
							<div className="flex-1">
								<a href="#">
									<p className="text-sm leading-6 font-medium text-gray-500">Terms of Service
										Privacy Policy
										Cookie Policy
										Accessibility
										Ads info
							...
							<div className="flex-2">
								<p className="text-sm leading-6 font-medium text-gray-600"> © 2022 Twitter, Inc.</p>
							</div>
						</div>
					</section>
				</div>
